...
class DataScaler():
    ...
    def scale_data(self, train_data, validation_data, test_data):
        if len(self.features_not_to_scale) == 0:
            ...
            categorical_features_test = test_data.loc[:, self.features_not_to_scale]
            continous_features_test = test_data.drop(labels=self.features_not_to_scale, axis=1)
            ...
            continous_test_scaled = self.scaler.transform(continous_features_test)
            ...
            test_scaled = np.concatenate((continous_test_scaled, categorical_features_test), axis=1)
