...
class Predictor():
    def __init__(self, model, criterion, path_data, path_results, path_residuals, columns_to_ignore):
        self.model = model
    ...
    def create_column_names_result(self):
        column_names_target = [column_name+" target" for column_name in self.column_names_data if column_name not in self.columns_to_ignore]
        column_names_predicted = [column_name+" predicted" for column_name in self.column_names_data if column_name not in self.columns_to_ignore]
        column_names_loss_per_sensor = [column_name+" share of loss " for column_name in self.column_names_data if column_name not in self.columns_to_ignore]
        return ["timestamp"] + column_names_target + column_names_predicted + ["loss"] + column_names_loss_per_sensor
    ...
    def predict(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for batch_number, data in enumerate(data_loader):
                ...
                data = [batch_number] + target_data_np + predicted_data_np + [loss.item()] + loss_share_per_sensor_np
